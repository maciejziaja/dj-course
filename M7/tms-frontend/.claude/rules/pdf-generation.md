...
All PDFs are produced by a 3-layer pipeline in `src/lib/pdf/` (per app). **Never call jsPDF
...
- `lib/pdf/` is **shared infra** per `vertical-slices.md` — slices import it, never fork it.
